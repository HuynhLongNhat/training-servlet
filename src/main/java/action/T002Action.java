 ...
 *
 * <p>
 * This servlet handles requests to view customers with pagination support. It
 * retrieves the total record count and current page of customers from
 * {@link T002Service}, then forwards the data to the JSP view for rendering.
 * </p>
 *
 * @author YourName
 * @version 1.0
...
public class T002Action extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// --- Check session: nếu chưa login thì redirect về login ---
		HttpSession session = request.getSession(false);

		if (session == null || session.getAttribute("user") == null) {
			response.sendRedirect(request.getContextPath() + "/T001"); // hoặc Constant.T001 nếu có
			return;
		}
		...
		try {
			...
			List<T002Dto> customers = t002Service.getAllCustomers(offset, PAGE_SIZE);
